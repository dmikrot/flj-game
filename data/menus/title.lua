...
function title_screen:phase_flj_presents()

  -- "Female Link Jam presents" displayed for two seconds
  self.phase = "flj_presents"

  self.flj_presents_img =
      sol.surface.create("title_screen_initialization.png", true)

  local width, height = self.flj_presents_img:get_size()
  self.flj_presents_pos = { 160 - width / 2, 120 - height / 2 }
...
function title_screen:on_draw(dst_surface)
  ...
  elseif self.phase == "flj_presents" then
...
function title_screen:draw_phase_present()
  ...
  self.flj_presents_img:draw(self.surface, self.flj_presents_pos[1], self.flj_presents_pos[2])
end
