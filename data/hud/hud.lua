...
function game:check_hud()
  ...
  if map ~= nil then
    ...
    local camera_x, camera_y, visible_x = map:get_camera_position()
    local x = visible_x - (hero_x - camera_x)
    ...
    elseif self.hud.showing_dialog and
      ...
      --self.hud.action_icon:set_dst_position(-67, 51)
      --self.hud.attack_icon:set_dst_position(-80, 29)
    end
  end
